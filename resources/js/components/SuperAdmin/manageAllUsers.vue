<template>
    <div class="col-12">
        <div class="bg-light rounded h-100 p-4">
            ...
            <!-- Pagination Controls -->
            <div class="pagination-controls" v-if="users.last_page > 1">
                <button
                    @click="fetchUserData(users.current_page - 1)"
                ...
                <button v-if="showPageButton(1)" @click="fetchUserData(1)" :class="{ 'active': 1 === users.current_page }" class="pagination-button">
                    1
                </button>
                ...
                <button v-if="showPageButton(users.last_page)" @click="fetchUserData(users.last_page)" :class="{ 'active': users.last_page === users.current_page }" class="pagination-button">
                    {{ users.last_page }}
                </button>
...
export default {
    ...
    setup() {
        ...
        const fetchUserData = async (page = 1) => {
            try {
                const [responseUsers, responseRoles, responseDepartments] =
                    await Promise.all([
                        axios.get("/api/getAllUsers", { params: { page } }),
                        axios.get("/api/getAllRoles"),
                        axios.get("/api/optionDepartments"),
                    ]);
                users.value = responseUsers.data;
                roles.value = responseRoles.data;
                departments.value = responseDepartments.data;
            } catch (error) {
        ...
        const fetchInitialData = async () => {
            try {
                const [rolesResponse, departmentsResponse, userResponse] =
                    await Promise.all([
                        axios.get("/api/getAllRoles"),
                        axios.get("/api/optionDepartments"),
        ...
        const updateRole = async (userId, roleId) => {
            try {
                await axios.put(`/api/updateUserRole/${userId}`, {
                    role_id: roleId,
                });
        ...
        const updateDepartment = async (userId, departmentId) => {
            try {
                await axios.put(`/api/updateUserDepartment/${userId}`, {
        ...
        const deleteUser = async (userId) => {
            try {
                ...
                fetchUserData(users.value.current_page); 
            } catch (error) {
        ...
        const shouldShowLeftEllipsis = computed(() => users.value.current_page > 4);
        const shouldShowRightEllipsis = computed(() => users.value.current_page < users.value.last_page - 3);
